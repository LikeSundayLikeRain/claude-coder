...
async def test_close_on_idle(lifecycle, bot):
    ...
    assert "idle" in bot.send_message.call_args.kwargs.get(
        "text", ""
    ).lower() or "idle" in str(bot.send_message.call_args)
    bot.close_forum_topic.assert_called_once_with(
        chat_id=-1001234, message_thread_id=42
    )
...
async def test_reopen(lifecycle, bot):
    ...
    bot.reopen_forum_topic.assert_called_once_with(
        chat_id=-1001234, message_thread_id=42
    )
...
async def test_rename_topic_calls_edit():
    ...
    bot = AsyncMock()
    await lifecycle.rename_topic(
        bot, chat_id=-1001234, message_thread_id=42, name="Fix auth bug"
    )
...
async def test_rename_topic_swallows_error():
    ...
    # Should not raise
    await lifecycle.rename_topic(
        bot, chat_id=-1001234, message_thread_id=42, name="test"
    )
